...
func runCompletion(binary string, args []string, stdout, stderr io.Writer) int {
	...
	shell := args[0]
	commands := []string{"bootstrap", "compile", "config", "completion", "deploy", "doctor", "events", "explain", "logs", "metrics", "object", "plan", "policy", "release", "rollback", "rollout", "saga", "state", "status", "validate", "version"}
