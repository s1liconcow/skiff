...
func runCompletion(binary string, args []string, stdout, stderr io.Writer) int {
	...
	shell := args[0]
	commands := []string{"authz", "bootstrap", "compile", "config", "completion", "deploy", "doctor", "events", "explain", "init", "logs", "metrics", "object", "ops", "plan", "plugin", "policy", "promote", "release", "rollback", "rollout", "saga", "solve", "state", "status", "tui", "validate", "version"}
