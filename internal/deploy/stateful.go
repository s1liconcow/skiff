...
func statefulRecommendedActions(group, operationID, traceID string) []StatefulRecommendedAction {
	return []StatefulRecommendedAction{
		...
		{ID: "recover_member", Command: commandWithTrace(fmt.Sprintf("skiff saga start stateful-replace --service %s --member <ordinal> --format json", group), traceID), Mutating: true, Safety: "requires explicit member fencing before volume attach", Reversibility: schema.Compensatable, Risk: schema.RiskHigh},
	}
}
