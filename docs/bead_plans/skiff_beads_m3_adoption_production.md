# Skiff Beads - Milestone 3: Adoption, Plugins, Recipes, Production Hardening, and GA

**Milestone:** M3 - Adoption and Production  
...
**Core product rule:** Keep Skiff easy to adopt without compromising the core model: object storage is durable truth, `skiffd` is the normal facade, direct CLI is recovery, and cloud primitives remain visible.

## Objective

Make Skiff adoptable and production-worthy. Add CI/CD release workflows, Terraform and Kubernetes adoption paths, plugins, optional mTLS, managed database recipes, database restore and rotation sagas, multi-region journeys, drift/GC, cost advisor, authorization, conformance tests, packaging, docs, and production readiness gates.

## Deliverable outcome

At the end of this milestone, Skiff is suitable for real customer pilots: teams can adopt from existing workflows, deploy API-plus-database stacks, perform operational sagas, extend the platform safely, and recover from common production incidents using documented commands.

## Dependency overlay

```text
Adoption chain:
  skiff-m3-001 ── skiff-m3-002
  skiff-m3-003
  skiff-m3-004

Composability chain:
  skiff-m3-005 ── skiff-m3-006
              └─ skiff-m3-016

Recipe/operations chain:
  skiff-m3-007 ── skiff-m3-008
              ├─ skiff-m3-009 ── skiff-m3-010
              └─ skiff-m3-011
  skiff-m3-012

Hardening chain:
  skiff-m3-013 ── skiff-m3-014
  skiff-m3-015 ── skiff-m3-017
  skiff-m3-016 ── skiff-m3-019 ── skiff-m3-020

Docs/demo chain:
  skiff-m3-018 depends on adoption, recipe, and packaging beads.
```

## Shared implementation conventions

- Adoption features should translate into Skiff concepts, not import entire external complexity models.
- Recipes should be explicit and honest about what is automated, what is reversible, and what requires approval.
- Plugins extend typed Skiff capabilities; they do not run arbitrary hidden controllers.
- CI/CD integrations author and approve releases; Skiff performs rollout, rollback, diagnosis, and event recording.
- Terraform can generate or own stable infrastructure shape; Skiff still owns releases, runtime operations, sagas, and state.
- Production hardening must produce machine-readable evidence that agents and humans can inspect.
