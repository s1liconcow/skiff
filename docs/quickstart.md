...
root object. Bootstrap defaults to the `development` class, where unsigned
local/dev code is allowed. Pass `--class production` or `--class staging` for
production-like controls. Skiff records that class and the resolved release
policy in config and object state; it does not infer production behavior from
environment names. As an operator guardrail, `--env prod` and
`--env production` require `--class production` or an explicit `--yes`
confirmation that the non-production class is intentional.
The managed private network includes a NAT gateway so private
...
`region`, object-state URI, `environmentClass`, `releasePolicy`, `writeRoleARN`,
and the live-apply setting from the `quickstart` context. For production-like
contexts, use `eval "$(skiff sudo JIRA-1234)"` before mutating commands to
assume the temporary write role and carry one `SKIFF_TRACE_ID` across the
session.
